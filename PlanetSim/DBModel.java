 ...
 // current simulation settings
	...
	public DBModel(String Name,int  StoragePrecision,int TemporalPrecision,int GeographicalPrecision, String StartDate, double Orbit, double Tilt, int GridSpacing, int TimeStep, int Length){
		...
		try {
			...
			this.conn = Connect(DATABASENAME);
			this.name = (Name == "")? "Sim_"+this.getTimeStamp() : Name;
	...
	private Connection Connect(String databaseName) throws Exception{
		 ...
		 if (this.isDebug()) { databaseName = databaseName.concat(getTimeStamp());	}
		 ...
		 try {
	          ...
	          if (this.isDebug()) {System.out.println("Creating:"+SIM_CONFIG_TBL);	}
	          stmt.execute(CREATE_SIM_CONFIG_TBL);								// creates table SIM_CONFIG_TBL if not present
	          if (this.isDebug()) {System.out.println("Creating:"+PLANET_CELLS_TBL);	}
		 ...
		 if (this.isDebug()){  System.out.println("Opened database successfully"); }
		    return c;	  
	}
	...
	private void cleanUp() throws SQLException{
		if (transactionCounter++ > MAX_NUMBER_TRANSACTION_COUNTER) { transactionCounter = 0; }
		if (transactionCounter % MAX_NUM_TRANS_TILL_COMMIT == 0){ this.conn.commit();}
		if (transactionCounter % ITER_FOR_GARBAGE_COLLECTION == 0){ System.gc();}
